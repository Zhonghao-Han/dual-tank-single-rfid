...
uint8_t sEE_WritePage(uint8_t *pBuffer, uint16_t WriteAddr,
                      uint8_t NumByteToWrite)  // FM441擦写eeprom完有中断，所以现在擦写eeprom也许注意中断屏蔽
{
    __disable_irq();
    if (!I2C_Start()) {
        __enable_irq();
    ...
    HAL_Delay(10);  // STOP后开始擦写，FM441修改为5ms，FM365为10ms, 貌似5ms有问题还是按照10ms来
    __enable_irq();
    return TRUE;
}
...
uint8_t FM11_WriteE2(uint16_t adr, uint32_t len, uint8_t *wbuf) {
    ...
    if (adr < FM11_E2_USER_ADDR || adr >= FM11_E2_MANUF_ADDR) return FALSE;
    if (adr % FM11_E2_BLOCK_SIZE)  // 求余 字节对齐
    {
        ...
        if (len > offset) {
            ...
            if (!sEE_WritePage(wbuf, adr, len)) return FALSE;
            len = 0;
        }
    }
    ...
    while (len) {
        if (len >= FM11_E2_BLOCK_SIZE) {
            if (!sEE_WritePage(wbuf, adr, FM11_E2_BLOCK_SIZE)) return FALSE;
...
uint8_t crc8(uint8_t *data, uint8_t data_length)  // 修改SAK必须要计算CRC8
{
    int i = 0;
    int j = 0;
    int Crc8 = 0xff;
